				...
				for (Staff staff : staffSet) {
					if (staff!=null) {
				        ...
				        if (staff.getFirstname()!=null){
					        ...
					        transformerHandle.characters(staff.getFirstname().toCharArray(), 0, staff.getFirstname().length());  
					        transformerHandle.endElement("", "", "firstname");
				        }
				        if (staff.getLastname()!=null){
				        	transformerHandle.startElement("", "", "lastname",attrImple);
					        transformerHandle.characters(staff.getLastname().toCharArray(), 0, staff.getLastname().length());  
					        transformerHandle.endElement("", "", "lastname");
				        }
				        if (staff.getNickname()!=null){
				        	transformerHandle.startElement("", "", "nickname",attrImple);
					        transformerHandle.characters(staff.getNickname().toCharArray(), 0, staff.getNickname().length());  
					        transformerHandle.endElement("", "", "nickname");
				        }
				        
				        transformerHandle.startElement("", "", "salary",attrImple);
					    transformerHandle.characters(Float.toString(staff.getSalary()).toCharArray(), 0, Float.toString(staff.getSalary()).length());  
